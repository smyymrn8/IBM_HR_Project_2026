...
# ========== MODEL 3: TOP10 HİPOTEZ BAZLI (SEGMENT) MODEL ==========
# hypothesis_pipeline.py içindeki fonksiyonunu kullanarak kolonları üretiyoruz
df_model3 = create_segments(df.copy())
...
target_hypotheses = [
    ...
    "salary_segment_risk",
    "H4_Satisfaction",
    "H16_Training_JobInvolvement",
    "H14_StockOption_JobLevel",
    "H3_Travel_WorkLife"
]

# Özellikleri ve hedefi seçme
X_h3 = df_model3[target_hypotheses]
y_h3 = df_model3["Attrition"]

# One-Hot Encoding (Risk -> 1, Other -> 0, Underpaid -> 1, Fair -> 0, High -> 0, Low -> 1)
X_h3_encoded = pd.get_dummies(X_h3, drop_first=True)

# Veri setini ayırma
X_train_h3, X_test_h3, y_train_h3, y_test_h3 = train_test_split(
    X_h3_encoded, y_h3, test_size=0.2, random_state=42, stratify=y_h3
)

# Ölçeklendirme (StandardScaler)
# MonthlyIncome ve TotalWorkingYears sayısal olduğu için scaling yapılmalı
scaler_3 = StandardScaler()
X_train_h3_scaled = scaler_3.fit_transform(X_train_h3)
X_test_h3_scaled = scaler_3.transform(X_test_h3)
...
print("\n--- MODEL 3: TOP 10 HİPOTEZ BAZLI (SEGMENT) MODEL SONUÇLARI ---")
print(classification_report(y_test_h3, y_pred_3))
print(f"ROC-AUC Skoru: {roc_auc_score(y_test_h3, y_prob_3):.4f}\n")



# ========== Katsayı analizi (Hangisi daha etkili?)
importance_h3 = pd.DataFrame({
    'Hypothesis': X_h3_encoded.columns,
    'Coefficient': log_model_3.coef_[0]
}).sort_values(by='Coefficient', ascending=False)

print("\n--- HİPOTEZLERİN ETKİ GÜCÜ (KATSAYILAR) ---")
print(importance_h3)



# ========== Hipotezlerin etkisini görselleştirelim
plt.figure(figsize=(10, 6))
sns.barplot(x='Coefficient', y='Hypothesis', data=importance_h3, palette='viridis')
plt.title('Hipotezlerin İşten Ayrılma Üzerindeki Etki Gücü')
